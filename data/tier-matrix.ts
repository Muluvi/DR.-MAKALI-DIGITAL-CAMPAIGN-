// the scope levels compared section "The service levels compared", transcribed exactly.
//
// Nine attributes across three tiers. Every string here is the cell as the scope levels compared section prints it — the
...
export interface TierAttribute {
  /** The row label, as the scope levels compared section prints it. */
  label: string;
  lean: Cell;
  standard: Cell;
  premium: Cell;
  /** Rendered bold in the scope levels compared section — the one cell in the matrix the source emphasises. */
...
export interface TierColumn {
  ...
  /** the three scope levels section's tier number, so the carousel and the budget modeller name the same thing. */
  number: string;
  /** the scope levels compared section's column heading. */
  label: string;
  recommended?: boolean;
}
