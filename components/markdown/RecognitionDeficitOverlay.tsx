...
/**
 * Section 6.5 — recognition deficit mapped against voter concentration.
 *
 * WHAT THIS DELIBERATELY DOES NOT SHOW: ward-level polling. No ward-level poll of this race
 * exists. Only two countywide Mizani data points are available (§1.1), and §6.6 lists
 ...
 *
 * What replaces them is what §6.5 actually asserts: a STRUCTURAL recognition status per
 ...
 * computed from data/ward-register.json rather than typed in; §6.5's own table is that same
...
/** Per §6.5: the anchor is Kitui Central, with Kitui West and Kitui Rural as the adjacent belt;
...
/** §6.5's "Total Decisive Deficit Pool" — Mwingi bloc plus Kitui South, both from the register. */
    ...
    <div className="my-8 bg-card border border-line rounded-2xl shadow-sm overflow-hidden not-prose">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 divide-y sm:divide-y-0 sm:divide-x divide-line border-b border-line bg-card">
        <div className="p-3.5 sm:p-4">
          <div className="text-[10px] uppercase font-black tracking-widest text-muted">Decisive deficit pool</div>
        ...
        <div className="p-3.5 sm:p-4">
          <div className="text-[10px] uppercase font-black tracking-widest text-muted">Countywide polling deficit</div>
          <div className="font-serif text-xl sm:text-2xl font-bold text-rose-600 dark:text-rose-400 mt-0.5 tabular-nums">
            −15.3 points
          </div>
          <div className="text-[11px] text-muted mt-0.5">Kasalu 37.4% · Mulu 22.1%</div>
        </div>
        <div className="p-3.5 sm:p-4">
          <div className="text-[10px] uppercase font-black tracking-widest text-muted">Phase −1 geofenced spend</div>
          <div className="font-serif text-xl sm:text-2xl font-bold text-accent mt-0.5 tabular-nums">65%</div>
          <div className="text-[11px] text-muted mt-0.5">Mwingi (all three) + Kitui South</div>
        </div>
      </div>
            ...
            <div key={`${w.constituencyId}-${w.name}`} className={`p-3 sm:p-3.5 rounded-xl border ${meta.className}`}>
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-2.5 min-w-0">
                  <span className="font-mono text-[11px] font-bold text-muted tabular-nums pt-0.5 shrink-0 w-5">
                    {w.rank}
                  </span>
                ...
                <div className="text-right shrink-0">
                  ...
                  <div className="text-[10px] text-muted tabular-nums">{share.toFixed(2)}% of register</div>
                </div>
              </div>
              <div className="flex items-center gap-1.5 mt-2.5 pt-2.5 border-t border-line/40">
                ...
                <span className="text-[10px] font-black uppercase tracking-wider text-ink">{meta.label}</span>
                <span className="text-[10px] text-muted truncate">— {meta.note}</span>
              </div>
            </div>
          );
        })}
