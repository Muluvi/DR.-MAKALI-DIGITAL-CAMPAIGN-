...
/**
 * the county's three regions section zone model, bound to the verified IEBC register.
 *
 ...
 *
 * Population, density and household figures are quoted from the urban and central anchor: Kitui Central and Kitui West section–1.4.3, which
 * sources them to the 2019 KNBS census. Phase weightings are quoted from the how the zones are weighted section.
 *
 * One honest wrinkle the UI has to carry rather than smooth over: the county's three regions section groups the county by
 ...
 * Kitui Rural — which the county's three regions section's three zones do not name at all — is surfaced as unassigned rather
...
interface GeographicZone {
  id: string;
  name: string;
  /** Sub-counties, exactly as the county's three regions section lists them. */
  subCounties: string;
...
/** the how the zones are weighted section's fourth row: held back for testing rather than allocated to a zone. */
      ...
      <div className="px-4 py-3 bg-paper/60 border-t border-line space-y-2">
        ...
        {UNASSIGNED.length > 0 && (
          <p className="t-small text-muted flex items-start gap-1.5">
            ...
            <span>
              The zones group the county by sub-county; the IEBC register is published by constituency, and the two do
