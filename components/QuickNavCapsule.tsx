      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="bg-card/95 backdrop-blur-xl border border-line/60 shadow-2xl rounded-2xl p-4 w-[min(calc(100vw-2.5rem),20rem)] mb-2 overflow-hidden"
          >
              ...
              {QUICK_TARGETS.map((item) => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center justify-between p-2.5 rounded-xl text-left hover:bg-accent/10 hover:text-accent group transition-all cursor-pointer text-xs font-bold text-ink min-h-[44px]"
                  >
      ...
      <div className="flex items-center gap-2">
        {showScrollTop && (
          <motion.button
            ...
            onClick={scrollToTop}
            className="w-11 h-11 rounded-full bg-card/90 backdrop-blur-md border border-line/60 shadow-lg text-muted hover:text-accent hover:border-accent active:scale-95 transition-all cursor-pointer flex items-center justify-center"
