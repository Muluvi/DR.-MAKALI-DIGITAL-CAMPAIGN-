...
import { Trophy, TrendingDown, Pause, Play, Compass, ArrowUpRight, CheckCircle2, ChevronRight, Gauge, LayoutGrid, List } from "lucide-react";
...
export function WardRegisterTicker() {
  ...
  const [viewType, setViewType] = useState<ViewType>("ticker");
 ...
 filterMode === "bottom"
          ...
          <div className="inline-flex p-0.5 bg-card border border-line/60 rounded-xl">
            <button
              ...
              title="Carousel stream view"
              aria-label="Carousel stream view"
            >
              <List size={14} />
            </button>
            <button
              ...
              title="40-Ward tile matrix view"
              aria-label="40-Ward tile matrix view"
            >
              <LayoutGrid size={14} />
            </button>
          </div>
      ...
      {/* Continuous Ticker Carousel OR Static 40-Ward Tile Matrix (Refinement 11) */}
      {viewType === "matrix" ? (
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-8 gap-2 py-2">
          {filteredWards.map((ward) => {
            const isSelected = selectedWard?.name === ward.name;
            return (
              <button
                key={`matrix-${ward.name}`}
                type="button"
                ...
                className={`p-2 rounded-xl text-left border transition-all cursor-pointer ${
                  isSelected
                    ...
                    ? "border-gold/40 bg-card hover:border-gold shadow-xs"
                    : ward.performanceTier === "bottom"
                    ? "border-line/70 bg-card hover:border-line"
                    : "border-line/60 bg-card hover:border-accent/40 shadow-xs"
                }`}
              >
                <div className="flex items-center justify-between text-muted t-micro font-mono">
                  <span className="font-bold">#{ward.rank}</span>
                  <span className="truncate max-w-[50px]">{ward.constituencyName.slice(0, 5)}</span>
                </div>
                <div className="font-bold text-ink t-label truncate mt-1">{ward.name}</div>
                <div className="font-mono t-micro text-accent font-semibold tnum mt-0.5">
                  {ward.voters.toLocaleString()}
                </div>
              </button>
            );
          })}
