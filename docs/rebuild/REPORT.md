...
in total) stays open as a `[CONFIRM/EDIT]`. R-1 to R-20 record what the rebuild found and changed.

## 6. What is still open

Generated by `scripts/build-open-items.ts` from every marker in the served content, every
`needed` figure in the data layer and every `needed` row in a figure: **44 data gaps, 3 items to
