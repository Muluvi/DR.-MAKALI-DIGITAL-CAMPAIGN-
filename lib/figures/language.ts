 ...
 * detail: a compressed banner at §7.3, the full matrix at §7.3.1, and a channel-by-channel
 * allocation at §7.3.4. The banner and the matrix are the same table — same three languages, same
 ...
 *
 * THE FIGURE'S REAL JOB IS C-21, and it is a compliance conflict rather than a rounding. §7.3.1
 ...
 * places in the document agree with §8.10.2, including §7.3.4 in the same subsection, which gives
 * the SMS rail 80% Kiswahili and 20% English and allocates Kikamba none of it.
 *
 * So the Kikamba channel list below is transcribed EXACTLY as §7.3.1 states it, bulk SMS included,
...
const LANGUAGE_REACH: Source = {
  name: "Stated in §7.3.1 as an estimated primary reach — source needed",
...
export type LanguageRow = {
  name: string;
  /** Estimated primary reach, as §7.3.1 states it. */
  share: number;
  audiences: string[];
  channels: string[];
  objective: string;
...
export const LANGUAGES: LanguageRow[] = [
  {
    ...
    // Transcribed as §7.3.1 states it. "2G Bulk SMS" is the conflict, not a typo to fix here.
...
export const LANGUAGE_SPLIT: FigureSeries = {
  ...
  measure: "Estimated primary reach per language, as §7.3.1 states it",
  ...
  conflicts: ["C-21"],
  note:
    "The three shares are stated in §7.3.1 without a citation and sum to 100%, so they are a " +
    "language map rather than a measurement. Kikamba's channel list is transcribed exactly as " +
    "§7.3.1 gives it, including 2G Bulk SMS — which §8.10.2 says the Communications Authority " +
    "closes to Kikamba. Neither line has been changed.",
};

/** §7.3.4's channel-by-channel allocation, whose SMS row is the version C-21 points at. */
export type DeploymentRow = {
  medium: string;
  primary: { language: string; share: number };
  secondary: { language: string; share: number }[];
  /** Where §7.3.4 wrote a qualifier in the cell rather than a percentage. */
  note?: string;
  conflicts?: string[];
};
...
export const LANGUAGE_DEPLOYMENT: DeploymentRow[] = [
  ...
  },
  {
    ...
    note: "Kiswahili rural, English for urban centres and youth boda. Kikamba is allocated none of this rail — which is what §8.10.2 requires and §7.3.1 contradicts.",
    conflicts: ["C-21"],
  },
  {
    ...
    note: "Toggle via option 0 on the USSD menu. USSD is not restricted — §8.10.2 names USSD menu text as a place Kikamba belongs.",
  },
  {
